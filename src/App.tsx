...
import { lessons, getCompletedLessonCount, getLesson, getRequiredLessonCount, isLessonUnlocked } from './content/chapters';
...
function initialLessonId(): LessonId {
  ...
  if (current && lessons.some((lesson) => lesson.id === current)) return current as LessonId;
  const legacy = localStorage.getItem('python-bite-class-current-v2') ?? localStorage.getItem('python-bite-class-current-v1');
  return (legacy?.startsWith('chapter-11-') ? 'chapter-11-1' : legacy && lessons.some((lesson) => lesson.id === legacy) ? legacy : lessons[0].id) as LessonId;
}
...
export default function App() {
  ...
  const navigate = (nextIndex: number) => { const next = lessons[nextIndex]; if (next && isLessonUnlocked(nextIndex, completed)) setCurrentId(next.id); };
