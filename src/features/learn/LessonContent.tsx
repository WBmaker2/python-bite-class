...
export function LessonContent({ lesson, completed, canComplete, onComplete, onPrevious, onNext, hasPrevious, hasNext }: Props) {
  ...
  return <article className="lesson-content">
    ...
    {lesson.challenge && <section className="challenge-block"><div className="challenge-heading"><span className="challenge-icon" aria-hidden="true">✦</span><div><span className="chapter-kicker">YOUR TURN</span><h3>도전 과제</h3></div></div><p>{lesson.challenge.prompt}</p><details><summary>힌트 보기</summary><p>{lesson.challenge.hint}</p></details></section>}
    {lesson.resources && <section className="resource-block" aria-labelledby="resource-title"><div className="concept-label">SOURCES</div><div><h3 id="resource-title">교재 출처와 라이선스</h3>{lesson.resources.map((resource) => <a href={resource.href} target="_blank" rel="noreferrer" key={resource.href}>{resource.label}<span>{resource.note ?? '↗'}</span></a>)}</div></section>}
    <footer className="lesson-actions"><button className="secondary-button" onClick={onPrevious} disabled={!hasPrevious}>← 이전</button><button className={`primary-button ${completed ? 'is-complete' : canComplete ? 'gi-pulse' : ''}`} onClick={onComplete} disabled={completed || !canComplete}>{completed ? '✓ 학습 완료됨' : canComplete ? '학습 완료' : lesson.completion === 'run' ? '코드를 먼저 실행해요' : lesson.completion === 'challenge' ? '도전을 통과해요' : '학습 완료'}</button><button className="secondary-button next-button" onClick={onNext} disabled={!hasNext}>다음 학습 →</button></footer>
  </article>;
}
