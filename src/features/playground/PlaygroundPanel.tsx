...
interface Props { lesson: Lesson; savedCode?: string; onCodeChange: (code: string) => void; onExecutionEvidence: (evidence: ExecutionEvidence) => void; onPracticeComplete: (complete: boolean) => void; }

export function PlaygroundPanel({ lesson, savedCode, onCodeChange, onExecutionEvidence, onPracticeComplete }: Props) {
  ...
  useEffect(() => { setCode(savedCode ?? lesson.starterCode ?? ''); setExecutedCode(undefined); reportedRunRef.current = ''; resetRunner(); }, [lesson.id]);
  ...
  useEffect(() => {
    const complete = lesson.completion === 'read' || lesson.completion === 'optional' || (lesson.completion === 'run' ? runner.state === 'done' && executedCode === code : lesson.completion === 'challenge' && challengeMessage?.passed === true && executedCode === code);
  ...
  }, [lesson.completion, runner.state, runner.outputs, challengeMessage, executedChallengeMessage, executedCode, code, onPracticeComplete, onExecutionEvidence]);
