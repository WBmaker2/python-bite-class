...
export const chapter1 = chapter(1, '이 프로그램 소개', [
  ...
  ], [{ term: '코딩', definition: '컴퓨터가 할 일을 명령어로 표현하는 활동' }]),
  ...
  ], [{ term: '출처 표시', definition: '누가 만든 자료인지 밝히는 약속' }, { term: 'CC BY-SA 4.0', definition: '저작자 표시와 동일조건변경허락을 요구하는 공개 라이선스' }], sources),
  makeLesson(1, 4, '첫 실행 준비', '설치 없이 브라우저에서 첫 문장을 실행하고 출력 창을 확인해 봅니다.', 'run', 'print("첫 실행 성공!")', '첫 실행 성공!', undefined, [
    { type: 'explanation', title: '안전한 실험실', body: '코드는 이 브라우저의 Python Worker 안에서만 실행됩니다. 이름이나 이메일을 입력할 필요가 없고, 실행 시간이 길어지면 자동으로 멈춰요.' },
    { type: 'example', title: '첫 명령', body: 'print()는 괄호 안의 글을 출력 창에 보여 줍니다.', code: 'print("첫 실행 성공!")' },
  ], [{ term: '출력', definition: '프로그램이 실행 결과로 보여 주는 내용' }]),
]);
