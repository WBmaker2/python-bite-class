...
const chapterLifeExample: Record<number, string> = {
  ...
  8: '자판기에 돈을 넣고 상품을 고르면 결과가 나오는 것처럼 함수의 입력과 출력을 읽어요.',
...
function makePracticeExample(chapterNumber: number, title: string, summary: string, starterCode: string, expectedOutput: string | undefined, challenge: Challenge | undefined): ConceptBlock {
  ...
  const firstLine = firstMeaningfulLine(starterCode);
  const output = expectedOutput ?? '실행한 뒤 출력창에서 확인해요.';
  return {
    type: 'example',
    title: '예제로 따라 하기',
    body: `${focus} 살펴봅니다. 시작 코드의 첫 부분(${firstLine})부터 읽으며 전체 흐름을 따라가요.\n생활 예시 → ${lifeExample}\n\n시작 코드의 예상 결과 → ${output}\n실행 → 코드 실행 버튼을 눌러요.\n관찰 → 출력창의 줄과 값을 시작 코드의 예상과 비교해요.\n${challenge ? `도전에서 바꿀 대상 → ${challenge.prompt}` : `따라 하기 → ${title}의 안내에 따라 시작 코드의 일부를 바꿔 봐요.`}`,
    code: starterCode,
  };
}

const practiceLoop = (expectedOutput: string | undefined, chapterNumber: number, challenge: Challenge | undefined) => `생활 예시 → ${chapterLifeExample[chapterNumber] ?? '주변에서 같은 규칙을 찾아 코드와 연결해요.'}\n시작 코드의 예상 결과 → ${expectedOutput ? `출력창에 다음 결과가 보여요:\n${expectedOutput}` : '실행한 뒤 출력창에서 결과를 확인해요.'}\n실행 → 코드 실행 버튼을 눌러요.\n관찰 → 출력 줄과 값을 예상과 비교해요.\n${challenge ? `도전에서 바꿀 대상 → ${challenge.prompt}` : '따라 하기 → 안내된 코드 일부를 한 번 바꿔 봐요.'}`;
...
function addChallengeGuidance(chapterNumber: number, title: string, challenge: Challenge): Challenge {
  ...
  return {
    ...
    commonMistakes: challenge.commonMistakes ?? [
      '코드 전체를 지우지 말고 안내된 값이나 줄만 바꿔요.',
      chapterNumber === 7 ? '조건의 콜론(:)과 들여쓰기는 그대로 두고, 안내된 비교 숫자나 범위만 바꿔요.' : '괄호·따옴표·들여쓰기가 시작 코드와 같은지 비교해요.',
...
export function makeLesson(
  ...
  const conceptsWithExample = completion !== 'read' && completion !== 'optional' && starterCode
    ? resolvedConcepts.some((concept) => concept.type === 'example')
      ...
      : [...resolvedConcepts, makePracticeExample(chapter, title, summary, starterCode, expectedOutput, challenge)]
    : resolvedConcepts;
  ...
  return {
    ...
    objectives: lessonGuide?.objectives ?? (completion === 'optional' ? ['원할 때 설명과 짧은 예제를 읽고 핵심 생각을 한 문장으로 정리할 수 있어요.'] : completion === 'read' ? ['설명을 읽고 핵심 생각을 한 문장으로 정리할 수 있어요.'] : ['예제 코드를 실행하고 결과를 관찰할 수 있어요.', '안내된 부분을 바꾸어 나만의 결과를 만들 수 있어요.']),
