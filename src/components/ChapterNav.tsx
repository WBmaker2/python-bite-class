...
export function ChapterNav({ currentId, completed, onSelect, onReset, open, onClose, onUpdates }: Props) {
  ...
  return <aside id="chapter-navigation" className={`chapter-nav ${open ? 'is-open' : ''}`} aria-label="학습 목차" aria-hidden={isMobile && !open}>
    ...
    <nav>{chapters.map((chapter) => { const chapterProgress = getChapterProgress(chapter, completed); const chapterLessonCount = getChapterLessonCount(chapter); return <section className="nav-chapter" key={chapter.number}>
      <button className={`nav-chapter-header ${chapter.number === chapters.find((item) => item.lessons.some((lesson) => lesson.id === currentId))?.number ? 'active' : ''}`} onClick={() => toggleChapter(chapter.number)} aria-expanded={expanded.has(chapter.number)}><span className="nav-number chapter-number">{chapterProgress === chapterLessonCount ? '✓' : chapter.number}</span><span className="nav-chapter-copy"><strong>{chapter.number}. {chapter.title}</strong><small>{chapterProgress}/{chapterLessonCount} 단계</small></span><span className="nav-chevron" aria-hidden="true">{expanded.has(chapter.number) ? '⌃' : '⌄'}</span></button>
      {expanded.has(chapter.number) && <div className="nav-lessons">{chapter.lessons.map((lesson) => { const index = lessons.findIndex((item) => item.id === lesson.id); const unlocked = isLessonUnlocked(index, completed); const done = completed.has(lesson.id); return <button key={lesson.id} className={`nav-item nav-lesson ${lesson.id === currentId ? 'active' : ''} ${!unlocked ? 'locked' : ''}`} aria-current={lesson.id === currentId ? 'step' : undefined} aria-disabled={!unlocked} disabled={!unlocked} onClick={() => { onSelect(lesson.id); onClose(); }}><span className={`nav-number ${done ? 'complete' : ''}`}>{done ? '✓' : unlocked ? `${chapter.number}.${lesson.order}` : '•'}</span><span><strong>{lesson.title}</strong><small>{done ? '완료했어요' : lesson.completion === 'optional' ? '선택 참고' : lesson.completion === 'read' ? '읽기 단계' : lesson.completion === 'run' ? '실행 단계' : '도전 단계'}</small></span></button>; })}</div>}
    </section>; })}</nav>
