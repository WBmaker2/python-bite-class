...
## 2026-09-11 제출 자료 관리 후속 변경

- 같은 제출 ID를 다시 보내면 Firestore의 동일 문서를 최신 스냅샷과 서버 시각으로 갱신하도록 바꾸었다. 다른 ID로 제출한 자료만 과거 기록으로 남는다.
- 익명 학생은 자기 UID의 제출 문서만 갱신할 수 있고, 교사 계정은 제출 문서를 건별로 삭제할 수 있도록 Rules를 최소 범위로 조정했다.
- 규칙 에뮬레이터 13개와 앱 테스트 113개를 통과한 뒤 `firebase-tools deploy --only firestore:rules --project python-bite-class-wbmaker01`로 운영 Rules를 다시 게시했다.
